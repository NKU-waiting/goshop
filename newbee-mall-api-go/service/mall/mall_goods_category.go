...
func (m *MallGoodsCategoryService) GetCategoriesForIndex() (err error, newBeeMallIndexCategoryVOS []mallRes.NewBeeMallIndexCategoryVO) {
	...
	// 1. 尝试从缓存获取
	found, err := cacheService.GetCategoryList(&newBeeMallIndexCategoryVOS)
	if err != nil {
		// Redis 错误不应该影响业务
		global.GVA_LOG.Warn("Redis error when getting category list", zap.Error(err))
	}

	if found && err == nil && len(newBeeMallIndexCategoryVOS) > 0 {
		// 缓存命中
		global.GVA_LOG.Debug("Cache hit for category list")
		return nil, newBeeMallIndexCategoryVOS
	}
	...
	// 3. 写入缓存
	if len(newBeeMallIndexCategoryVOS) > 0 {
		if cacheErr := cacheService.SetCategoryList(newBeeMallIndexCategoryVOS); cacheErr != nil {
			global.GVA_LOG.Warn("Failed to set category list cache", zap.Error(cacheErr))
		}
	}
